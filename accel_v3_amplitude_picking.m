function varargout = accel_v3_amplitude_picking(varargin)
% ACCEL_V3_AMPLITUDE_PICKING MATLAB code for accel_v3_amplitude_picking.fig
%      ACCEL_V3_AMPLITUDE_PICKING, by itself, creates a new ACCEL_V3_AMPLITUDE_PICKING or raises the existing
%      singleton*.
%
%      H = ACCEL_V3_AMPLITUDE_PICKING returns the handle to a new ACCEL_V3_AMPLITUDE_PICKING or the handle to
%      the existing singleton*.
%
%      ACCEL_V3_AMPLITUDE_PICKING('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in ACCEL_V3_AMPLITUDE_PICKING.M with the given input arguments.
%
%      ACCEL_V3_AMPLITUDE_PICKING('Property','Value',...) creates a new ACCEL_V3_AMPLITUDE_PICKING or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before accel_v3_amplitude_picking_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to accel_v3_amplitude_picking_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help accel_v3_amplitude_picking

% Last Modified by GUIDE v2.5 21-Nov-2016 22:33:12

% Begin initialization code - DO NOT EDIT
gui_Singleton = 0;
gui_State = struct('gui_Name',       mfilename, ...
    'gui_Singleton',  gui_Singleton, ...
    'gui_OpeningFcn', @accel_v3_amplitude_picking_OpeningFcn, ...
    'gui_OutputFcn',  @accel_v3_amplitude_picking_OutputFcn, ...
    'gui_LayoutFcn',  [] , ...
    'gui_Callback',   []);
...
% --- Executes just before accel_v3_amplitude_picking is made visible.
function accel_v3_amplitude_picking_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to accel_v3_amplitude_picking (see VARARGIN)

% Choose default command line output for accel_v3_amplitude_picking
handles.output = hObject;
% Update handles structure
guidata(hObject, handles);

% UIWAIT makes accel_v3_amplitude_picking wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = accel_v3_amplitude_picking_OutputFcn(hObject, eventdata, handles)
...
if isempty(get(handles.Import_c40,'UserData'))
    ...
    set(handles.Fine_tune_encoder,'Enable','off');
    set(handles.Import_c40,'Enable','off');
    msgbox('Aligning encoder successful!');
    return;
end
...
Plot_NSE_Callback(handles.Plot_NSE, eventdata, handles);
...
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
x = getcursor(handles,1);
y = getcursor(handles,2);
picktime = clock;
SN = getSN(handles);
Arrival = zeros(3,4);
cellevent = {'SN','pickTime','startEnd','Arrivals';...
    SN,picktime,[x y],Arrival};
...
function picks = getcursor(handles, xORy)
try
    dcm_obj = datacursormode(handles.figure1);
    c_info = getCursorInfo(dcm_obj);
    picks = c_info.Position(xORy);
catch ME
...
if exist([AccelFile.AccelPath, 'DB_amp_pick_', dryname(1:end-2), '.mat'],'file')
    % Construct a questdlg with three options
    choice = questdlg('A data file already exists.', ...
        'What would you like to do?', ...
        'Delete the file and I will save again.',...
        'Only update the events.','Cancel','Cancel');
    % Handle response
    switch choice
        case 'Delete the file and I will save again.'
            delete([AccelFile.AccelPath, 'DB_amp_pick_',...
                dryname(1:end-2), '.mat']);
            msgbox('File successfully deleted!');
        case 'Only update the events.'
            eventsDB = get(handles.eventlist,'UserData');
            save([AccelFile.AccelPath, 'DB_amp_pick_', dryname(1:end-2), '.mat'],...
                'eventsDB','-append');
            msgbox('Update successful! (Only events were saved)');
        case 'Cancel'
            return;
    end    
else
    ...
    try
        save([AccelFile.AccelPath, 'DB_amp_pick_', dryname(1:end-2), '.mat'],...
            'Accel','ROGA','eventsDB','test','AccelFile','ROGAFile','-v7.3');
...
T = get(handles.eventlist,'UserData');
if isempty(T); return; end
pick = getcursor(handles,2);    % picks y value of the cursor
i = get(handles.AccelCh,'Value');
card = floor((i-1)/3)+1;
ch = mod(i-1,3)+1;
eventNo = get(handles.eventlist,'Value');
